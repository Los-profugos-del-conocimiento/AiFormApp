...
const SidebarFormCard = ({ id, title, type, handleCloseSidebarMobile }: SidebarFormCardProps) => {
  ...
  return (
    <>
      <div className="w-full flex flex-row">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className=" bg-slate-800 border-black">
            ...
            <DropdownMenuItem className="text-red-500 focus:bg-slate-500 focus:text-red-500" onClick={() => setIsDeleteModalOpen(true)}>
              <MdDelete size={23} />
              Eliminar
            </DropdownMenuItem>
          </DropdownMenuContent>
      ...
      <DeleteFormModal title={title} isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)}/>
    </>
  );
};
